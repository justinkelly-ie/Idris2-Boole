...
--
-- A type alias for a Multiset with BF2 coefficients.
...
||| BIT-GATE MULTISET: A Multiset with BF2 (mod-2) coefficients.
||| Addition natively performs XOR: equal states cancel (1+1=0).
public export
BitGateMset : (state : Type) -> Type
BitGateMset state = Multiset BF2 state
...
||| Insert a state with a given BF2 weight into the bit-gate multiset.
public export
insertBit : Eq state => state -> BF2 -> BitGateMset state -> BitGateMset state
...
public export
evaluateState : Eq state => BitGateMset state -> state -> BF2
evaluateState ZeroM _ = Z
evaluateState (AddM k v rest) s =
  if k == s
    then addBF2 v (evaluateState rest s)
...
liftBitGateToBoxInt (AddM k v rest) = AddM k (bf2ToBoxInt v) (liftBitGateToBoxInt rest)
